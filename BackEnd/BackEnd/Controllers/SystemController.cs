...
namespace BackEnd.Controllers
{
    ...
    public class SystemController : ControllerBase
    {
        ...
        {
            // In a real system, this would query from a logging/monitoring system
        ...
        public IActionResult GetPerformance()
        {
            ...
            {
                Hour = $"{i}:00",
                Value = random.Next(30, 80) // Mock response time in ms
            }).ToList();

            var requestVolumeData = Enumerable.Range(0, 24).Select(i => new
            {
                Hour = $"{i}:00",
                Value = random.Next(8000, 13000) // Mock request count
            }).ToList();
